...
int configure(int fd, struct termios *saved)
{
    ...
    /* Blocking read mode (VMIN=1/VTIME=0): return as soon as one byte
       arrives.  The timed mode (VMIN=0/VTIME=1) proved to lose short
       command frames on this hardware, while the continuous GPRMC stream
       still came through. */
...
int run(const std::string &device, const command_handler &handler,
        ...
        const stray_bytes_sink &stray_sink)
{
    ...
    while (!should_stop()) {
        ...
        if (ready == 0) {
            if (!input.empty()) {
                if (!stray_sink || input.front() == kRequestHeader) {
                    if (send_length_error(fd) != OK) {
                        result_value = ERR_IO;
                        break;
                    }
                } else {
                    stray_sink(std::move(input));
                }
                input.clear();
            }
            continue;
        ...
        while (!input.empty()) {
            ...
            if (header != input.begin() && stray_sink) {
                stray_sink(std::vector<std::uint8_t>(input.begin(), header));
            }
