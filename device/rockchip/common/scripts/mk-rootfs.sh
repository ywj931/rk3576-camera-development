...
build_debian()
{
	...
	if [ ! -f linaro-$RK_DEBIAN_VERSION-alip-*.tar.gz ]; then
		RELEASE=$RK_DEBIAN_VERSION TARGET=desktop ARCH=$ARCH \
			RK_DEBIAN_MIRROR=$RK_DEBIAN_MIRROR \
			./mk-base-debian.sh
...
build_ubuntu()
{
	...
	ARCH=${RK_UBUNTU_ARCH:-arm64}

	# A second rootfs build would remove the first build's chroot mounts.
	mkdir -p "$RK_OUTDIR"
	exec 9>"$RK_OUTDIR/.ubuntu-rootfs.lock"
	if ! flock -n 9; then
		error "Another Ubuntu rootfs build is already running in this workspace."
		exit 1
	fi
	...
	message "=========================================="
	message "          Start building Ubuntu $RK_UBUNTU_VERSION($ARCH)"
	message "=========================================="
	...
	# Download and prepare the base Ubuntu rootfs tarball
	ARCH=$ARCH ./mk-base-ubuntu.sh

	UBUNTU_SCRIPT=mk-rootfs-$RK_UBUNTU_VERSION.sh

	VERSION=release ARCH=$ARCH RK_UBUNTU_MIRROR=$RK_UBUNTU_MIRROR ./$UBUNTU_SCRIPT
	./mk-image.sh
	...
	ln -rsf "$PWD/ubuntu-rootfs.img" "$IMAGE_DIR/rootfs.ext4"
