...
// X-DAS shutter slot 1 is 16 seconds. Sensor modes may clamp long exposures.
constexpr uint32_t kMaxExposureUs = 16000000;
...
extern "C" int camera_backend_set_max_exposure(camera_backend_t *backend,
                                                int camera_id,
                                                uint32_t max_exposure_us)
{
    ...
    if (result != CAMERA_BACKEND_OK)
        return result;
    if (max_exposure_us < kMinExposureUs ||
        max_exposure_us > kMaxExposureUs)
        return CAMERA_BACKEND_ERR_RANGE;
    ...
    result = record_aiq_result(
        camera, "getExpSwAttr(max-exposure)",
        rk_aiq_user_api2_ae_getExpSwAttr(camera.ctx, &attribute));
    if (result != CAMERA_BACKEND_OK)
        return result;

    attribute.advanced.sw_aeT_advAeRange_en = true;
    attribute.advanced.linExpRange.sw_aeT_time_max =
        static_cast<float>(max_exposure_us) / 1000000.0f;
    result = record_aiq_result(
        camera, "setExpSwAttr(max-exposure)",
        rk_aiq_user_api2_ae_setExpSwAttr(camera.ctx, attribute));
    if (result == CAMERA_BACKEND_OK) {
        std::fprintf(stdout,
                     "CAMERA_MAX_EXPOSURE camera_id=%d max_exposure_us=%u state=APPLIED\n",
                     camera_id, max_exposure_us);
        std::fflush(stdout);
    }
    return result;
}

extern "C" int camera_backend_get_max_exposure(camera_backend_t *backend,
                                                int camera_id,
                                                uint32_t *max_exposure_us)
{
    if (!max_exposure_us)
    ...
    ae_api_expSwAttr_t attribute = {};
    result = record_aiq_result(
        camera, "getExpSwAttr(max-exposure-query)",
        rk_aiq_user_api2_ae_getExpSwAttr(camera.ctx, &attribute));
    if (result != CAMERA_BACKEND_OK)
        return result;
    const float seconds =
    ...
    if (!std::isfinite(seconds) || seconds <= 0.0f ||
        seconds > static_cast<float>(kMaxExposureUs) / 1000000.0f) {
        return CAMERA_BACKEND_ERR_VERIFY;
    }
    const uint32_t value = to_u32_rounded(seconds * 1000000.0f);
    if (value < kMinExposureUs || value > kMaxExposureUs)
        return CAMERA_BACKEND_ERR_VERIFY;
    *max_exposure_us = value;
    return CAMERA_BACKEND_OK;
}
