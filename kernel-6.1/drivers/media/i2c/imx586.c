...
#define IMX586_REG_GROUP_HOLD		0x0104
#define IMX586_GROUP_HOLD_START		0x1
#define IMX586_GROUP_HOLD_END		0x0

#define IMX586_REG_FRAME_LENGTH_CTRL	0x0350
#define IMX586_FRAME_LENGTH_AUTO	0x1
#define IMX586_REG_XVS_IO_CTRL		0x3040
#define IMX586_XVS_INPUT		0x0
#define IMX586_REG_MASTER_SLAVE_SEL	0x3041
#define IMX586_SLAVE_MODE		0x0
#define IMX586_REG_INTERNAL_XVS_OFFSET_LINE	0x3f68
#define IMX586_REG_INTERNAL_XVS_OFFSET_VTPXCK	0x3f6a
#define IMX586_REG_INPUT_XVS_MULT_SEL	0x3f6e
#define IMX586_REG_INPUT_XVS_MULT	0x3f6f
#define IMX586_REG_MC_MODE		0x3f70
#define IMX586_MC_MODE_FIXED		0x1
#define IMX586_REG_XVS_OUTPUT_ENABLE	0x3f71
#define IMX586_XVS_OUTPUT_DISABLE	0x0
#define IMX586_REG_PRSH_LENGTH_LINES	0x3f79
#define IMX586_PRSH_MARGIN_LINES	34
#define IMX586_REG_XVS_INPUT_POLARITY	0x4b85
#define IMX586_XVS_LOW_ACTIVE		0x1
...
static int imx586_set_group_hold(struct imx586 *imx586, bool hold)
{
	return imx586_write_reg(imx586->client, IMX586_REG_GROUP_HOLD,
				IMX586_REG_VALUE_08BIT,
				hold ? IMX586_GROUP_HOLD_START :
				       IMX586_GROUP_HOLD_END);
}

static int imx586_write_prsh_length(struct imx586 *imx586, u32 exposure)
{
	u32 prsh_length = exposure + IMX586_PRSH_MARGIN_LINES;

	return imx586_write_reg(imx586->client,
				IMX586_REG_PRSH_LENGTH_LINES,
				IMX586_REG_VALUE_24BIT, prsh_length);
}

static int imx586_write_exposure(struct imx586 *imx586, u32 exposure)
{
	int ret;
	int release_ret;

	if (imx586->xvs_slave_mode) {
		ret = imx586_set_group_hold(imx586, true);
		if (ret)
			return ret;
	}

	ret = imx586_write_reg(imx586->client, IMX586_REG_EXPOSURE_H,
			       IMX586_REG_VALUE_16BIT, exposure);
	if (!ret && imx586->xvs_slave_mode)
		ret = imx586_write_prsh_length(imx586, exposure);

	if (imx586->xvs_slave_mode) {
		release_ret = imx586_set_group_hold(imx586, false);
		if (!ret)
			ret = release_ret;
	}
...
static int imx586_write_analogue_gain(struct imx586 *imx586, u32 gain)
{
	int ret;
	int release_ret;
...
static int imx586_configure_xvs_slave(struct imx586 *imx586)
{
	u32 exposure = imx586->exposure ? imx586->exposure->val :
					 imx586->cur_mode->exp_def;
	int ret;

	if (!imx586->xvs_slave_mode)
		return 0;

	/* These registers must be programmed while the sensor is in standby. */
	ret = imx586_write_reg(imx586->client, IMX586_REG_MC_MODE,
			       IMX586_REG_VALUE_08BIT, IMX586_MC_MODE_FIXED);
	ret |= imx586_write_reg(imx586->client, IMX586_REG_MASTER_SLAVE_SEL,
				IMX586_REG_VALUE_08BIT, IMX586_SLAVE_MODE);
	ret |= imx586_write_reg(imx586->client, IMX586_REG_XVS_IO_CTRL,
				IMX586_REG_VALUE_08BIT, IMX586_XVS_INPUT);
	ret |= imx586_write_reg(imx586->client, IMX586_REG_XVS_OUTPUT_ENABLE,
				IMX586_REG_VALUE_08BIT,
				IMX586_XVS_OUTPUT_DISABLE);
	ret |= imx586_write_reg(imx586->client, IMX586_REG_XVS_INPUT_POLARITY,
				IMX586_REG_VALUE_08BIT, IMX586_XVS_LOW_ACTIVE);
	ret |= imx586_write_reg(imx586->client, IMX586_REG_FRAME_LENGTH_CTRL,
				IMX586_REG_VALUE_08BIT, IMX586_FRAME_LENGTH_AUTO);
	ret |= imx586_write_reg(imx586->client,
				IMX586_REG_INTERNAL_XVS_OFFSET_LINE,
				IMX586_REG_VALUE_16BIT, 0);
	ret |= imx586_write_reg(imx586->client,
				IMX586_REG_INTERNAL_XVS_OFFSET_VTPXCK,
				IMX586_REG_VALUE_16BIT, 0);
	ret |= imx586_write_reg(imx586->client,
...
static int __imx586_start_stream(struct imx586 *imx586)
{
	...
	ret = imx586_configure_xvs_slave(imx586);
	if (ret) {
		dev_err(&imx586->client->dev,
			"failed to configure XVS slave mode: %d\n", ret);
		return ret;
	}
...
static int imx586_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_EXPOSURE:
		ret = imx586_write_exposure(imx586, ctrl->val);
	...
	case V4L2_CID_ANALOGUE_GAIN:
		...
		ret = imx586_write_analogue_gain(imx586, again);
