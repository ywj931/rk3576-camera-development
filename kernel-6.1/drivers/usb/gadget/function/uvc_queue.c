...
#if defined(CONFIG_ARCH_ROCKCHIP) && defined(CONFIG_NO_GKI)
		/*
		 * RK3576 boards can expose memory above the 32-bit DMA window.
		 * vb2_dma_sg_alloc() prefers large physically contiguous chunks;
		 * when such a chunk lands above 4 GiB, DWC3 falls back to SWIOTLB.
		 * A 4000x3000 MJPEG buffer (4 MiB) exceeds SWIOTLB's maximum
		 * single mapping size and VIDIOC_REQBUFS then fails with -ENOMEM.
		 * Keep UVC output buffers in the DMA zone so DWC3 maps them
		 * directly instead of requiring a bounce buffer.
		 */
		queue->queue.gfp_flags = GFP_DMA;
#endif
		queue->use_sg = 1;
	} else {
