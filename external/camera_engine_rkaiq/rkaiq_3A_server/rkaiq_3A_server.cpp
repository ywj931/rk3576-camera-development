...
static int init_engine(struct rkaiq_media_info *media_info)
{
    ...
    if (media_info->aiq_ctx == NULL) {
        ERR("%s: AIQ init failed for sensor %s\n",
            media_info->mdev_path, media_info->sensor_entity_name);
        return -1;
    }
    ...
    ret = rk_aiq_uapi2_sysctl_prepare(media_info->aiq_ctx, width, height,
                                     RK_AIQ_WORKING_MODE_NORMAL);
    if (ret != XCAM_RETURN_NO_ERROR) {
        ERR("%s: AIQ prepare %dx%d failed: %d\n",
            media_info->mdev_path, width, height, ret);
...
static int start_engine(struct rkaiq_media_info *media_info)
{
    XCamReturn ret;

    if (media_info->aiq_ctx == NULL) {
        ERR("%s: cannot start an uninitialized AIQ context\n", media_info->mdev_path);
        return -1;
    }

    DBG("device manager start\n");
    ret = rk_aiq_uapi2_sysctl_start(media_info->aiq_ctx);
    if (ret != XCAM_RETURN_NO_ERROR) {
        ERR("%s: AIQ start failed: %d\n", media_info->mdev_path, ret);
        return -1;
    }
...
void parse_args(int argc, char **argv)
{
   ...
   while (1) {
       ...
       c = getopt_long(argc, argv, "sw:H:h", long_options, &option_index);
       if (c == -1)
           break;
       ...
       case 's':
           ...
           break;
       case 'h':
           ERR("Usage: %s [--silent] [--width pixels] [--height pixels]\n",
               argv[0]);
           exit(0);
       case '?':
           ERR("Usage: %s [--silent] [--width pixels] [--height pixels]\n",
