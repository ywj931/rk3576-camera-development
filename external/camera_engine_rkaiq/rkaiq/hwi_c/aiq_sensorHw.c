...
AiqSensorExpInfo_t* SensorHw_getEffectiveExpParams(AiqSensorHw_t* pSnsHw, uint32_t frame_id) {
    ...
    if (!pItem) {
        ...
        if (pItem) {
            LOGD_CAMHW_SUBM(SENSOR_SUBM,
                            "cam%d use effecting exposure of %d for %d, map size %d",
                            pSns->mCamPhyId, (uint32_t)(long)(pItem->_key), search_id,
                            aiqMap_size(pSns->_effecting_exp_map));
        } else {
            /*
             * Keep the legacy C++ fallback: when the map is not empty but
             * contains only entries newer than search_id, use the newest
             * available exposure instead of returning NULL.
             */
            pItem = aiqMap_rbegin(pSns->_effecting_exp_map);
            if (pItem) {
                AiqMapItem_t* pFirstItem = aiqMap_begin(pSns->_effecting_exp_map);
                LOGW_CAMHW_SUBM(
                    SENSOR_SUBM,
                    "cam%d fallback exposure %d for %d, map size %d, range [%d, %d]",
                    pSns->mCamPhyId, (uint32_t)(long)(pItem->_key), search_id,
                    aiqMap_size(pSns->_effecting_exp_map),
                    pFirstItem ? (uint32_t)(long)(pFirstItem->_key) : 0,
                    (uint32_t)(long)(pItem->_key));
            } else {
                LOGE_CAMHW_SUBM(
                    SENSOR_SUBM,
                    "cam%d can't find effecting exposure for id %d, map is empty",
                    pSns->mCamPhyId, search_id);
                aiqMutex_unlock(&pSns->_mutex);
                return NULL;
            }
        }
