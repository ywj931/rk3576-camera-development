...
/* Resolve the UTC of the XVS trigger (the sync board's PPS) that drove the
   frame whose V4L2 frame-end monotonic timestamp is frame_monotonic_ns.
   The frame ends roughly a quarter second after its own trigger, so the
   most recent whole-second PPS boundary at or before the frame end is the
   trigger for this frame's exposure. */
int gnss_time_source_trigger_utc_ns(
