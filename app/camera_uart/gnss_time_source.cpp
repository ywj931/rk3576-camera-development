...
constexpr uint32_t kDefaultMaxRmcDelayMs = 900;
                        ...
                        size_t length)
{
    ...
    for (;;) {
        ...
        if (!monotonic_now_ns(&arrival_ns)) {
            ++source->status.time_mapping_errors;
            continue;
        }
...
{
    if (!source || source->uart_device.empty() || source->pps_device.empty())
        return GNSS_TIME_ERR_ARGUMENT;
    {
        std::lock_guard<std::mutex> lock(source->mutex);
                                            ...
                                            size_t report_capacity)
{
    ...
    if (result == GNSS_TIME_OK)
        result = gnss_time_source_inject_pps(source, 100, 9000000000ULL,
                                             1000000000ULL);
    if (result == GNSS_TIME_OK)
        result = gnss_time_source_inject_nmea(source, rmc.c_str(),
                                              1100000000ULL);
    if (result == GNSS_TIME_OK)
    ...
    std::snprintf(report, report_capacity,
                  "direct_gnss=%s pps=%llu rmc=%llu invalid=%llu unpaired=%llu utc_ns=%llu",
